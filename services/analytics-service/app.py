...
if not AWS_REGION:
    log.critical("Erro: AWS_REGION deve ser definido.")
    sys.exit(1)

if not DYNAMODB_TABLE_NAME:
    log.critical("Erro: AWS_DYNAMODB_TABLE deve ser definido.")
    sys.exit(1)

# SQS é opcional para desenvolvimento local
if not SQS_QUEUE_URL:
    log.warning("Atenção: AWS_SQS_URL não definida. O worker SQS não será iniciado.")
...
try:
    ...
    # Suporte para endpoint customizado (útil para DynamoDB Local)
    endpoint_url = os.getenv("AWS_ENDPOINT_URL")
    if endpoint_url:
        if SQS_QUEUE_URL:
            sqs_client = session.client("sqs", endpoint_url=endpoint_url)
        dynamodb_client = session.client("dynamodb", endpoint_url=endpoint_url)
    else:
        if SQS_QUEUE_URL:
            sqs_client = session.client("sqs")
        dynamodb_client = session.client("dynamodb")
    log.info(f"Clientes Boto3 inicializados na região {AWS_REGION}" + (f" (endpoint: {endpoint_url})" if endpoint_url else ""))
...
def sqs_worker_loop():
    ...
    if not SQS_QUEUE_URL:
        log.info("Worker SQS não iniciado: AWS_SQS_URL não definida")
        return
